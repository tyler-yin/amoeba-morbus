...
class Microbe {
  ...
  PVector separate (ArrayList<Microbe> microbes) {
    ...
    for (Microbe other : microbes) {
      ...
      //!!!!!!!!! FUNCTIONALITY !!!!!!!!!// 
      if ((d > 500 - virusSpread) && (d < 500 + 1.25*virusSpread) 
        //&& (location.x > width/3 && location.x < 2*width/3) 
        //&& (location.y > height/3 && location.y < 2*height/3)
        ) {
        panic = true;
      } else {
        panic = false;
  ...
  void borders() {
    ...
    //if (location.x < width/4-50) location.x = 3*width/4+50;
    //if (location.y < -r-15) location.y = height+r;
    //if (location.x > 3*width/4+50) location.x = width/4-50;
    //if (location.y > height+r+15) location.y = -r;
  }
